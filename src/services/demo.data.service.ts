import { VotingItemData } from "../models/VotingitemData.model";


// Sample Voting Items Data
const sampleVotingItems1: VotingItemData[] = [
  { votingItemId: "item1", votingItemOrdinal: 1, description: "Option A", voteCount: 0 },
  { votingItemId: "item2", votingItemOrdinal: 2, description: "Option B", voteCount: 0 },
  { votingItemId: "item3", votingItemOrdinal: 3, description: "Option C", voteCount: 0 },
];

const sampleVotingItems2: VotingItemData[] = [
  { votingItemId: "item1", votingItemOrdinal: 1, description: "Option X", voteCount: 0 },
  { votingItemId: "item2", votingItemOrdinal: 2, description: "Option Y", voteCount: 0 },
  { votingItemId: "item3", votingItemOrdinal: 3, description: "Option Z", voteCount: 0 },
];

const sampleVotingItems3: VotingItemData[] = [
  { votingItemId: "item1", votingItemOrdinal: 1, description: "Choice 1", voteCount: 0 },
  { votingItemId: "item2", votingItemOrdinal: 2, description: "Choice 2", voteCount: 0 },
  { votingItemId: "item3", votingItemOrdinal: 3, description: "Choice 3", voteCount: 0 },
];

// Sample Polls Data
const samplePolls = [
  {
    pollId: "051908e4-eacb-45a0-8956-a86763eada3d",
    title: "Favorite Fruit",
    description: "Select your favorite fruit.",
    nofAnswersAllowed: 1,
    creatorId: "219b3c35-f850-4fec-8cc0-01ddfd80ef11",
    groupId: "group1",
    timeCreated: "2023-01-01T00:00:00Z",
    timeUpdated: "2023-01-01T00:00:00Z",
    deadline: "2023-12-31T23:59:59Z",
    votingItems: sampleVotingItems1,
  },
  {
    pollId: "7b0857a6-f59a-42d2-98fe-d5eef162d4d0",
    title: "Preferred Programming Language",
    description: "Choose your preferred programming language.",
    nofAnswersAllowed: 2,
    creatorId: "219b3c35-f850-4fec-8cc0-01ddfd80ef11",
    groupId: "group2",
    timeCreated: "2023-02-01T00:00:00Z",
    timeUpdated: "2023-02-01T00:00:00Z",
    deadline: "2023-11-30T23:59:59Z",
    votingItems: sampleVotingItems2,
  },
  {
    pollId: "618212e5-e9a3-427d-8ac3-0580e0223f08",
    title: "Best Movie Genre",
    description: "Select your favorite movie genre.",
    nofAnswersAllowed: 1,
    creatorId: "219b3c35-f850-4fec-8cc0-01ddfd80ef11",
    groupId: "group3",
    timeCreated: "2023-03-01T00:00:00Z",
    timeUpdated: "2023-03-01T00:00:00Z",
    deadline: "2023-10-31T23:59:59Z",
    votingItems: sampleVotingItems3,
  },
];

// Function to get sample polls
export function getSamplePolls() {
  return samplePolls;
}